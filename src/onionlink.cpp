...
[[maybe_unused]] Bytes sha256(const Bytes &in) {
    Bytes out(32);
    require(mbedtls_sha256(in.data(), in.size(), out.data(), 0) == 0, "sha256 failed");
    return out;
}

Bytes sha3_256(const Bytes &in) {
    Bytes out(32);
    require(mbedtls_sha3(MBEDTLS_SHA3_256, in.data(), in.size(), out.data(), out.size()) == 0,
            "sha3 failed");
    return out;
}
...
[[maybe_unused]] Bytes hmac_sha256(const Bytes &key, const Bytes &msg) {
    ...
    require(mbedtls_md_hmac(info, key.data(), key.size(), msg.data(), msg.size(), out.data()) == 0,
            "hmac failed");
    return out;
}
...
Bytes shake256(const Bytes &in, size_t out_len) {
    Bytes out(out_len);
    require(crypto_xof_shake256(out.data(), out.size(), in.data(), in.size()) == 0,
            "shake256 failed");
    return out;
}
...
Bytes aes_ctr_crypt(const Bytes &key, const Bytes &input, const Bytes &iv = Bytes(16, 0)) {
    require(iv.size() == 16, "bad aes ctr iv");
    Bytes out(input.size());
    mbedtls_aes_context ctx;
    mbedtls_aes_init(&ctx);
    int rc = mbedtls_aes_setkey_enc(&ctx, key.data(), static_cast<unsigned int>(key.size() * 8));
    require(rc == 0, "aes setkey failed");
    std::array<uint8_t, 16> nonce{};
    std::array<uint8_t, 16> stream{};
    std::copy(iv.begin(), iv.end(), nonce.begin());
    size_t nc_off = 0;
    rc = mbedtls_aes_crypt_ctr(&ctx, input.size(), &nc_off, nonce.data(), stream.data(),
                               input.data(), out.data());
...
class AesCtrStream {
  public:
    ...
    void init(const Bytes &key) {
        key_ = key;
        initialized_ = true;
        require(mbedtls_aes_setkey_enc(&ctx_, key.data(), static_cast<unsigned int>(key.size() * 8)) == 0,
    ...
    Bytes apply(const Bytes &input) {
        Bytes out(input.size());
        require(mbedtls_aes_crypt_ctr(&ctx_, input.size(), &nc_off_, nonce_.data(), stream_.data(),
                                      input.data(), out.data()) == 0,
                "aes stream failed");
        return out;
    }
  ...
  private:
    void move_from(AesCtrStream &other) noexcept {
        ...
        if (!key_.empty()) {
            initialized_ = true;
            (void) mbedtls_aes_setkey_enc(&ctx_, key_.data(), static_cast<unsigned int>(key_.size() * 8));
        }
        nonce_ = other.nonce_;
...
class Sha1Digest {
  public:
    ...
    void update(const Bytes &b) {
        require(mbedtls_sha1_update(&ctx_, b.data(), b.size()) == 0, "sha1 update failed");
    }
    Bytes current() const {
...
class Sha3Digest {
  public:
    Sha3Digest() {
        ...
        require(mbedtls_sha3_starts(&ctx_, MBEDTLS_SHA3_256) == 0, "sha3 starts failed");
    }
    ...
    ~Sha3Digest() {
        mbedtls_sha3_free(&ctx_);
    }
    void update(const Bytes &b) {
        require(mbedtls_sha3_update(&ctx_, b.data(), b.size()) == 0, "sha3 update failed");
    }
    Bytes current() const {
        ...
        Bytes out(32);
        require(mbedtls_sha3_finish(&tmp, out.data(), out.size()) == 0, "sha3 finish failed");
...
class RelayCrypto {
  public:
    ...
    RelayCrypto() = default;
    RelayCrypto(const Bytes &df, const Bytes &db, const Bytes &kf, const Bytes &kb, DigestKind kind)
    ...
    Bytes encrypt_relay(uint8_t relay_cmd, uint16_t stream_id, const Bytes &data) {
    ...
    Bytes encrypt_body_only(const Bytes &body) {
        return f_.apply(body);
    }

    Bytes decrypt_body_only(const Bytes &body) {
        return b_.apply(body);
    }
...
Bytes base64_decode(std::string s) {
    ...
    size_t bin_len = 0;
    int rc = sodium_base642bin(out.data(), out.size(), s.c_str(), s.size(), nullptr, &bin_len,
...
std::string base64_encode_unpadded(const Bytes &b) {
    size_t max_len = sodium_base64_encoded_len(b.size(), sodium_base64_VARIANT_ORIGINAL);
    std::string out(max_len, '\0');
    sodium_bin2base64(out.data(), out.size(), b.data(), b.size(), sodium_base64_VARIANT_ORIGINAL);
...
OnionAddress parse_onion_address(const std::string &addr) {
    ...
    require(checksum[0] == expected[0] && checksum[1] == expected[1], "bad onion checksum");
    require(crypto_core_ed25519_is_valid_point(pub.data()) == 1, "onion ed25519 key is invalid");
...
int connect_tcp(const std::string &host, uint16_t port, int timeout_ms) {
    ...
    require(gai == 0, std::string("getaddrinfo failed for ") + host + ": " + gai_strerror(gai));
    int fd = -1;
    ...
    freeaddrinfo(res);
    require(fd >= 0, "tcp connect failed to " + host + ":" + std::to_string(port));
    timeval tv{};
...
Bytes decode_http_body(const Bytes &response) {
    ...
    require(head.find(" 200 ") != std::string::npos, "HTTP request failed: " + head.substr(0, head.find("\r\n")));
    ...
    if (lhead.find("transfer-encoding: chunked") != std::string::npos) {
        ...
        while (p < body.size()) {
            ...
            std::string len_s(reinterpret_cast<const char *>(body.data() + p), line_end - p);
...
Bytes http_get_direct(const HostPort &hp, const std::string &path, int timeout_ms) {
...
[[maybe_unused]] std::string after_prefix(const std::string &line, const std::string &prefix) {
...
Consensus parse_consensus(const std::string &doc) {
    ...
    while (std::getline(in, line)) {
        ...
        } else if (parts[0] == "id" && cur && parts.size() >= 3 && parts[1] == "ed25519" &&
                   parts[2] != "none") {
...
Relay parse_microdescriptor_into(Relay relay, const std::string &doc) {
    ...
    require(relay.ntor_key.size() == 32, "microdescriptor missing ntor-onion-key for " + relay.nickname);
    return relay;
}
...
std::map<std::string, MicrodescriptorFields> index_microdescriptors(const std::string &raw) {
    ...
    for (const std::string &doc : split_microdescriptors(raw)) {
        ...
        out[base64_encode_unpadded(sha256(bytes))] = parse_microdescriptor_fields(doc);
    }
    return out;
}
...
bool relay_usable_hsdir(const Relay &r) {
    return relay_usable_dir(r) && r.has_flag("HSDir") && !r.has_flag("NoEdConsensus") &&
           r.ed_id.size() == 32;
}

bool relay_usable_rendezvous(const Relay &r) {
    return relay_usable_dir(r) && !r.has_flag("MiddleOnly") && !r.has_flag("NoEdConsensus") &&
           r.rsa_id.size() == 20 && r.ed_id.size() == 32 && r.ntor_key.size() == 32;
}
...
Bytes blind_onion_key(const Bytes &pubkey, uint64_t period_num, uint64_t period_len) {
    ...
    h[31] |= 64;
    Bytes blinded(32);
    int rc = crypto_scalarmult_ed25519_noclamp(blinded.data(), h.data(), pubkey.data());
...
HsPeriodKeys derive_hs_period_keys(const Consensus &c, const OnionAddress &addr) {
    HsPeriodKeys k;
    k.period_len = c.param("hsdir-interval", 1440);
    k.period_num = current_period_num(c, k.period_len);
    k.blinded = blind_onion_key(addr.pubkey, k.period_num, static_cast<uint64_t>(k.period_len));
    k.subcredential = onion_subcredential(addr.pubkey, k.blinded);
    return k;
}

std::vector<Relay> select_hsdirs(const Consensus &c, const Bytes &blinded, const Bytes &srv,
                                 uint64_t period_num, int period_len) {
    ...
    std::sort(ring.begin(), ring.end(), [](const Indexed &a, const Indexed &b) {
        return a.idx < b.idx;
    });
    ...
    for (int rep = 1; rep <= replicas; ++rep) {
        ...
        auto it = std::lower_bound(ring.begin(), ring.end(), service_idx,
                                   [](const Indexed &a, const Bytes &idx) { return a.idx < idx; });
        size_t start = static_cast<size_t>(it - ring.begin());
        for (size_t n = 0, seen = 0; seen < ring.size() && n < static_cast<size_t>(spread); ++seen) {
...
std::vector<LinkSpecifier> parse_link_specifiers(const Bytes &encoded) {
    ...
    for (uint8_t i = 0; i < n; ++i) {
        ...
        specs.push_back({type, Bytes(encoded.begin() + static_cast<long>(pos),
...
std::optional<HostPort> link_spec_ipv4(const std::vector<LinkSpecifier> &specs) {
    for (const auto &s : specs) {
...
std::vector<LinkSpecifier> relay_link_specifiers(const Relay &r) {
    std::vector<LinkSpecifier> specs;
    in_addr a{};
    require(inet_pton(AF_INET, r.ip.c_str(), &a) == 1, "relay has non-IPv4 address");
    Bytes ipv4(reinterpret_cast<uint8_t *>(&a), reinterpret_cast<uint8_t *>(&a) + 4);
...
bool same_relay(const Relay &a, const Relay &b) {
    ...
    return !a.ip.empty() && a.or_port != 0 && a.ip == b.ip && a.or_port == b.or_port;
}
...
class TorChannel {
  public:
    ...
    Cell read_cell() {
        return read_cell_with_circ_len(4);
    }
  ...
  private:
    void init_tls() {
        ...
        require(mbedtls_net_connect(&net_, host_.c_str(), std::to_string(port_).c_str(),
                                    MBEDTLS_NET_PROTO_TCP) == 0,
                "TLS tcp connect failed to " + host_ + ":" + std::to_string(port_));
        require(mbedtls_ssl_config_defaults(&conf_, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM,
        ...
        mbedtls_ssl_set_bio(&ssl_, &net_, mbedtls_net_send, mbedtls_net_recv, mbedtls_net_recv_timeout);
        int rc = 0;
...
RelayCrypto finish_ntor(const NtorState &st, const Bytes &hdata) {
    ...
    Bytes k = hkdf_sha256_expand(key_seed, from_string("ntor-curve25519-sha256-1:key_expand"), 92);
...
class Circuit {
  public:
    ...
    static Circuit create_fast(std::shared_ptr<TorChannel> ch) {
        ...
        for (;;) {
            ...
            require(c.cmd == CMD_CREATED_FAST, "unexpected cell while waiting for CREATED_FAST");
    ...
    void send_raw_body(const Bytes &body) {
        require(body.size() == kCellBodyLen, "raw relay body must be one cell body");
    ...
    void extend_ntor(const Relay &relay) {
        ...
        for (;;) {
            ...
            if (m.cmd == RELAY_EXTENDED2) {
                ...
                require(m.data.size() >= 2u + static_cast<size_t>(hlen), "truncated EXTENDED2 handshake");
  ...
  private:
    void send_relay_cell(uint8_t cell_cmd, size_t hop_index, uint8_t relay_cmd, uint16_t stream_id,
                         const Bytes &data) {
    ...
    static RelayMessage parse_relay_body(const Bytes &body) {
        ...
        m.data.assign(body.begin() + kRelayHeaderLen, body.begin() + kRelayHeaderLen + len);
        return m;
    }
...
[[maybe_unused]] Bytes begin_dir_get(const Relay &relay, const std::string &path, int timeout_ms) {
    ...
    for (size_t off = 0; off < rb.size(); off += kRelayPayloadLen) {
        ...
        circ.send_relay(RELAY_DATA, 1, Bytes(rb.begin() + static_cast<long>(off),
...
Bytes begin_dir_get_via(const Relay &guard, const Relay &target, const std::string &path, int timeout_ms) {
...
std::string extract_pem_message(const std::string &text, const std::string &begin_label,
...
Bytes parse_ed25519_cert_subject(const std::string &pem) {
    Bytes cert = base64_decode(extract_pem_message(pem, "-----BEGIN ED25519 CERT-----",
                                                   "-----END ED25519 CERT-----"));
...
Bytes decrypt_descriptor_layer(const Bytes &ciphertext, const Bytes &secret_data,
    ...
    secret_input.insert(secret_input.end(), subcredential.begin(), subcredential.end());
...
Relay intro_relay_from_descriptor(const IntroductionPoint &intro) {
    ...
    require(r.rsa_id.size() == 20, "intro point lacks RSA identity link specifier");
    require(r.ed_id.size() == 32, "intro point lacks Ed25519 identity link specifier");
...
HiddenServiceDescriptor parse_inner_descriptor(const std::string &plain) {
    ...
    while (std::getline(in, line)) {
        ...
        } else if (parts[0] == "onion-key" && cur && parts.size() >= 3 && parts[1] == "ntor") {
            cur->ntor_key = base64_decode(parts[2]);
        } else if (parts[0] == "enc-key" && cur && parts.size() >= 3 && parts[1] == "ntor") {
    ...
    desc.intros.erase(std::remove_if(desc.intros.begin(), desc.intros.end(), [](const IntroductionPoint &ip) {
                         return ip.auth_key.size() != 32 || ip.ntor_key.size() != 32 ||
                                ip.enc_key.size() != 32 ||
...
HiddenServiceDescriptor decrypt_hs_descriptor(const std::string &outer, const HsPeriodKeys &keys) {
    uint64_t revision = 0;
    ...
    require(revision != 0 || outer.find("revision-counter 0") != std::string::npos,
            "descriptor missing revision-counter");
    require(!super_pem.empty(), "descriptor missing superencrypted blob");
    Bytes super_cipher = base64_decode(extract_pem_message(super_pem, "-----BEGIN MESSAGE-----",
                                                           "-----END MESSAGE-----"));
    Bytes first_plain = decrypt_descriptor_layer(super_cipher, keys.blinded, keys.subcredential, revision,
                                                 "hsdir-superencrypted-data");
    std::string first = to_string_lossy(first_plain);
    std::string inner_pem = extract_pem_message(first, "-----BEGIN MESSAGE-----", "-----END MESSAGE-----");
    Bytes inner_cipher = base64_decode(inner_pem);
    Bytes second_secret = keys.blinded;
    Bytes second_plain = decrypt_descriptor_layer(inner_cipher, second_secret, keys.subcredential, revision,
...
Bytes x25519_public_from_private(const Bytes &priv) {
    ...
    Bytes pub(32);
    require(crypto_scalarmult_base(pub.data(), priv.data()) == 0, "x25519 public failed");
    return pub;
}
...
Bytes x25519_shared(const Bytes &priv, const Bytes &pub) {
    ...
    Bytes out(32);
    require(crypto_scalarmult(out.data(), priv.data(), pub.data()) == 0, "x25519 shared secret failed");
    return out;
}

HsIntroPayload build_introduce1(const IntroductionPoint &intro, const Relay &rp,
                                const Bytes &rend_cookie, const HsPeriodKeys &keys) {
    ...
    intro_secret.insert(intro_secret.end(), intro.auth_key.begin(), intro.auth_key.end());
...
RelayCrypto finish_hs_ntor(HsNtorState &st, const Bytes &handshake_info) {
    ...
    secret.insert(secret.end(), Y.begin(), Y.end());
    secret.insert(secret.end(), proto.begin(), proto.end());
    Bytes ntor_key_seed = tor_mac(secret, from_string(std::string(kHsProto) + ":hs_key_extract"));
    Bytes verify = tor_mac(secret, from_string(std::string(kHsProto) + ":hs_verify"));
    Bytes auth_input = verify;
    ...
    Bytes server = from_string("Server");
    auth_input.insert(auth_input.end(), server.begin(), server.end());
    Bytes expected = tor_mac(auth_input, from_string(std::string(kHsProto) + ":hs_mac"));
...
class RendezvousStream {
  public:
    ...
    void send_data(uint16_t stream_id, const Bytes &data) {
        for (size_t off = 0; off < data.size(); off += kRelayPayloadLen) {
            ...
            send_hs(RELAY_DATA, stream_id,
                    Bytes(data.begin() + static_cast<long>(off), data.begin() + static_cast<long>(off + n)));
        }
    }
    ...
    void end(uint16_t stream_id) {
        send_hs(RELAY_END, stream_id, Bytes{6});
    }
...
std::string fetch_microdescriptor_doc(const HostPort &bootstrap, const Relay &r, int timeout_ms) {
    require(!r.md_digest.empty(), "relay missing microdescriptor digest");
    Bytes body = http_get_direct(bootstrap, "/tor/micro/d/" + r.md_digest, timeout_ms);
    return to_string_lossy(body);
}

void hydrate_microdescriptors(Consensus &consensus, const HostPort &bootstrap, int timeout_ms, bool verbose) {
    std::vector<std::string> digests;
    std::set<std::string> seen;
    for (const Relay &r : consensus.relays) {
        if (!relay_usable_dir(r) || r.md_digest.empty() || r.has_flag("NoEdConsensus")) {
            continue;
        }
        if (r.has_flag("HSDir")) {
    ...
    require(!digests.empty(), "consensus has no microdescriptor digests to fetch");
    ...
    std::shuffle(sources.begin() + 1, sources.end(), std::mt19937{std::random_device{}()});
    ...
    size_t worker_count = std::min<size_t>(8, std::max<size_t>(1, batches.size()));
    auto worker = [&](size_t worker_id) {
        for (;;) {
            ...
            if (verbose) {
                ...
                std::cerr << "fetching HSDir microdescriptors " << batch.first << "-" << batch.last
                          << " of " << digests.size() << "\n";
            }
            bool ok = false;
            ...
            if (!ok && verbose) {
                std::lock_guard<std::mutex> lock(log_mu);
                std::cerr << "microdescriptor batch " << batch.first << "-" << batch.last
                          << " failed after " << attempts << " sources: " << last_error << "\n";
            }
        }
    };
...
DescriptorFetchResult fetch_hidden_service_descriptor(const Consensus &consensus, const HsPeriodKeys &keys,
    ...
    for (const Bytes &srv : srvs) {
        std::vector<Relay> hsdirs = select_hsdirs(consensus, keys.blinded, srv, keys.period_num, keys.period_len);
        std::shuffle(hsdirs.begin(), hsdirs.end(), std::mt19937{std::random_device{}()});
        size_t guard_pos = 0;
        for (const Relay &hsdir : hsdirs) {
            try {
                ...
                return DescriptorFetchResult{decrypt_hs_descriptor(to_string_lossy(body), keys), guard};
            } catch (const std::exception &e) {
                last_error = e.what();
            }
        }
...
Bytes read_stdin_all() {
    return Bytes(std::istreambuf_iterator<char>(std::cin), std::istreambuf_iterator<char>());
}
...
Consensus load_consensus(const Options &opt) {
    ...
    if (opt.verbose) {
        std::cerr << "fetching microdescriptor consensus from " << opt.bootstrap.host << ":"
                  << opt.bootstrap.port << "\n";
    }
    Bytes doc = http_get_direct(opt.bootstrap, "/tor/status-vote/current/consensus-microdesc", opt.timeout_ms);
    return parse_consensus(to_string_lossy(doc));
}

[[maybe_unused]] Relay choose_rendezvous_with_ntor(const Consensus &consensus, const HostPort &bootstrap,
                                                   int timeout_ms, bool verbose) {
    ...
    for (Relay r : candidates) {
        try {
            ...
            r = parse_microdescriptor_into(r, fetch_microdescriptor_doc(bootstrap, r, timeout_ms));
            return r;
        } catch (const std::exception &e) {
            last_error = e.what();
        }
...
RendezvousStream connect_onion_service(const Options &opt, const Consensus &consensus,
    ...
    if (opt.verbose) {
        std::cerr << "connecting to rendezvous point " << rp.nickname << " at " << rp.ip << ":"
                  << rp.or_port << " via guard " << guard.nickname << "\n";
    }
    auto rp_ch = std::make_shared<TorChannel>(guard.ip, guard.or_port, opt.timeout_ms);
    ...
    std::shuffle(intros.begin(), intros.end(), std::mt19937{std::random_device{}()});
    std::string last_error;
    ...
    for (const auto &intro : intros) {
        try {
            ...
            auto ip_ch = std::make_shared<TorChannel>(guard.ip, guard.or_port, opt.timeout_ms);
            ...
            for (;;) {
                ...
                if (ack.cmd == RELAY_INTRODUCE_ACK) {
                    ...
                    require(status == 0, "INTRODUCE_ACK status " + std::to_string(status));
...
RendezvousStream connect_onion_service_with_retries(const Options &opt, const Consensus &consensus,
...
int main(int argc, char **argv) {
    try {
        ...
        if (opt.verbose) {
            std::cerr << "derived blinded key " << base64_encode_unpadded(keys.blinded)
                      << " for period " << keys.period_num << "\n";
        }
        hydrate_microdescriptors(consensus, opt.bootstrap, opt.timeout_ms, opt.verbose);
        DescriptorFetchResult desc =
            fetch_hidden_service_descriptor(consensus, keys, opt.timeout_ms, opt.verbose);
        RendezvousStream stream =
            connect_onion_service_with_retries(opt, consensus, desc.descriptor, keys, {desc.guard});
